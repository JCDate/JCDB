   ...
   public void agregar(Connection conexion, ProveedoresM prov) throws SQLException {
       try {
           ...
           consulta.executeUpdate();
            ...
            // Crear consulta preparada para obtener los registros a actualizar
            String sqlId = "SELECT id_prov FROM proveedores WHERE nombre=? AND calibre=?";
            PreparedStatement statement = conexion.prepareStatement(sqlId);
            statement.setString(1, prov.getNombre());
            statement.setString(2, prov.getCalibre());   
            ResultSet rs = statement.executeQuery();
            int id = 0;
            // Recorrer los resultados y ejecutar la actualización para cada registro
            while (rs.next()) {
                id = rs.getInt("id_prov");
            }

           PreparedStatement consulta2;
           consulta2 = conexion.prepareStatement("INSERT INTO inventario (calibre,descripcion,num_calibre,unidad,milesimas,existencia,kgs,tolerancia,peso_hoja,observaciones,id_prov) "
                   + "SELECT   calibre,descripcion,num_calibre,unidad,milesimas,existencia,kgs,tolerancia,peso_hoja,observaciones, ? as id_prov FROM inventario WHERE calibre=?"); 
           consulta2.setInt(1, id);
           consulta2.setString(2, prov.getCalibre());
