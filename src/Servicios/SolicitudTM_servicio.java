...
public class SolicitudTM_servicio {
    ...
    public void agregar(Connection conexion, SolicitudTMPDM sol) throws SQLException {
        try {
            ...
            consulta = conexion.prepareStatement("INSERT INTO " + this.tabla + "(cotizacion,componente, fechaSol,envioCot,precioC,enviadoSKF,aprobadoSKF,solicitudM,costoM,fechaMaq,temple,ajuste,produccion,pzFacturadaSKF,aprobacion) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public void modificar(Connection conexion, SolicitudTMPDM sol) throws SQLException {
        try {
            ...
            consulta = conexion.prepareStatement("UPDATE " + this.tabla + " SET  componente=?, fechaSol=?,envioCot=?,precioC=?,enviadoSKF=?,aprobadoSKF=?,solicitudM=?,costoM=?,fechaMaq=?,temple=?,ajuste=?,produccion=?,pzFacturadaSKF=?, aprobacion=? WHERE cotizacion = ?");
            ...
            consulta.setBoolean(14, sol.isAprobacion());
            consulta.setString(15, sol.getCotizacion());
    ...
    public void eliminar(Connection conexion, SolicitudTMPDM sol) throws SQLException {
        try {
            PreparedStatement consulta;
            consulta = conexion.prepareStatement("DELETE FROM " + this.tabla + " WHERE cotizacion = ?");
            consulta.setString(1, sol.getCotizacion());
            consulta.executeUpdate();
