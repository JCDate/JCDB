...
public class CRsGUI extends javax.swing.JFrame {
    ...
    public CRsGUI(Usuarios mod) throws SQLException, ClassNotFoundException {
        ...
        PreparedStatement pst = conexion.prepareStatement("INSERT INTO antecedentesfamilia (antecedentesfamilia.componente) SELECT crs.componente FROM crs WHERE NOT EXISTS ( SELECT antecedentesfamilia.componente FROM antecedentesfamilia WHERE antecedentesfamilia.componente = crs.componente)");
        pst.executeUpdate();
        //estructura
        PreparedStatement pst2 = conexion.prepareStatement("UPDATE estructura JOIN crs ON crs.componente = estructura.componente SET estructura.CR = crs.cr");
        pst2.executeUpdate();
        //dimensiones
        PreparedStatement pst3 = conexion.prepareStatement("INSERT INTO dim (dim.componente, dim.cr) SELECT crs.componente, crs.cr FROM crs WHERE NOT EXISTS ( SELECT dim.componente FROM dim WHERE dim.componente = crs.componente)");
        pst3.executeUpdate();
        //dimensiones cr
        PreparedStatement pst6 = conexion.prepareStatement("UPDATE dim JOIN crs ON crs.componente = dim.componente SET dim.cr = crs.cr");
        pst6.executeUpdate();
        //Calculo
        PreparedStatement pst5 = conexion.prepareStatement("UPDATE datoslistaprecios JOIN crs ON crs.componente = datoslistaprecios.componente SET datoslistaprecios.cr = crs.cr");
        pst5.executeUpdate();

        //C/R EMBARQUE
        PreparedStatement pst7 = conexion.prepareStatement("UPDATE embarque JOIN crs ON crs.componente = embarque.componente SET embarque.cr = crs.cr");
        pst7.executeUpdate();
    ...
    private void CRs() {
        try {
            this.crs = this.crs_servicio.recuperarTodas(Conexion.getInstance().getConnection());
