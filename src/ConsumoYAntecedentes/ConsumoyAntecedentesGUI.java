...
public class ConsumoyAntecedentesGUI extends javax.swing.JFrame {
    ...
    public ConsumoyAntecedentesGUI(Usuarios mod) throws SQLException, ClassNotFoundException {
        ...
        try {
            //cr
            PreparedStatement pst = conexion.prepareStatement("INSERT INTO crs (crs.componente) SELECT consumoyantecedentes.componente_CA FROM consumoyantecedentes WHERE NOT EXISTS ( SELECT crs.componente FROM crs WHERE crs.componente = consumoyantecedentes.componente_CA)");
            pst.executeUpdate();

            //Ancho de tira
            PreparedStatement pst2 = conexion.prepareStatement("UPDATE datostecnicosmp JOIN consumoyantecedentes ON consumoyantecedentes.componente_CA = datostecnicosmp.componente SET datostecnicosmp.anchoTira_p = consumoyantecedentes.anchoTira");
            pst2.executeUpdate();

            //Precio componente
            PreparedStatement pst3 = conexion.prepareStatement("INSERT INTO preciocomponente (preciocomponente.componente) SELECT consumoyantecedentes.componente_CA FROM consumoyantecedentes WHERE NOT EXISTS ( SELECT preciocomponente.componente FROM preciocomponente WHERE preciocomponente.componente = consumoyantecedentes.componente_CA)");
            pst3.executeUpdate();

            //Estructura componente
            PreparedStatement pst4 = conexion.prepareStatement("INSERT INTO estructura (estructura.componente) SELECT consumoyantecedentes.componente_CA FROM consumoyantecedentes WHERE NOT EXISTS ( SELECT estructura.componente FROM estructura WHERE estructura.componente = consumoyantecedentes.componente_CA)");
            pst4.executeUpdate();

            //Estructura consumo unitario
            PreparedStatement pst5 = conexion.prepareStatement("UPDATE estructura JOIN consumoyantecedentes ON consumoyantecedentes.componente_CA = estructura.componente SET estructura.peso_estamp = consumoyantecedentes.consumo_uni");
            pst5.executeUpdate();

            //Calibre dimensiones
            PreparedStatement pst6 = conexion.prepareStatement("UPDATE dim JOIN consumoyantecedentes ON consumoyantecedentes.componente_CA = dim.componente SET dim.calibre = consumoyantecedentes.desMatePrima");
            pst6.executeUpdate();

            //Inspeccion Almacen
            PreparedStatement pst7 = conexion.prepareStatement("INSERT INTO almaceninspeccion (componente) SELECT consumoyantecedentes.componente_CA FROM consumoyantecedentes WHERE consumoyantecedentes.tipo='ALTO VOLUMEN' AND NOT EXISTS ( SELECT almaceninspeccion.componente FROM almaceninspeccion WHERE almaceninspeccion.componente = consumoyantecedentes.componente_CA)");
            pst7.executeUpdate();
    ...
    private void ConsumoyAntecedentesGUI() {
        try {
            this.CA = this.consumoyAntecedentes_servicio.recuperarTodas(Conexion.getInstance().getConnection());
    ...
    private void jTable1KeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTable1KeyReleased
        if (evt.getKeyCode() == com.sun.glass.events.KeyEvent.VK_ENTER) {
            try {
                ...
                consulta = conexion.prepareStatement("UPDATE consumoyantecedentes SET consumo_uni='" + consumo_uni + "',desMatePrima='" + desMatePrima + "',anchoTira='" + anchoTira + "',pesoPzs='" + pesoPzs + "',PpapStatus='" + PpapStatus + "',comentario_CA='" + comentario_CA + "',tipo='" + tipo + "' WHERE componente_CA='" + componente + "'");
